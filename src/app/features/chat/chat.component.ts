...
export class ChatComponent implements AfterViewChecked, OnInit, OnDestroy {
  ...
  private async initCryptoKey(): Promise<void> {
    try {
      // Ensure RSA key pair exists (loads from IndexedDB or generates new one)
      await this.cryptoSvc.ensureRSAKeyPair();
      // Generate AES session key for outgoing messages
    ...
    this.chatApi
      ...
      .subscribe((me) => {
        ...
        if (this.currentUserId) {
          ...
          void this.registerRSAPublicKey(this.currentUserId);
        }
      });
  }

  private async registerRSAPublicKey(userId: number): Promise<void> {
    try {
      const pubPem = await this.cryptoSvc.getPublicPem();
      await lastValueFrom(
        this.chatApi.setUserPublicKey(userId, pubPem).pipe(
          catchError((err) => {
            console.warn('Failed to register RSA public key:', err);
            return of(null);
          }),
        ),
      );
      console.log('RSA public key registered with backend.');
    } catch (err) {
      console.error('Failed to register RSA public key:', err);
    }
  }
    ...
    if (this.aesKey && target === 'direct') {
      ...
      try {
        ...
        if (!recipientPubPem) {
          ...
          // Use the original CryptoKey (not the exported ArrayBuffer) to avoid
          // potential ArrayBuffer detachment issues after the first wrap call.
          let wrappedForSender: string | null = null;
          try {
            if (this.currentUserId != null && this.aesKey) {
              const myPubPem = await this.cryptoSvc.getPublicPem();
              if (myPubPem) {
                wrappedForSender = await this.cryptoSvc.wrapAESKeyForRecipient(this.aesKey as CryptoKey, myPubPem);
  ...
  private mergeSocketMessage(
    ...
    // For own messages with empty content (encrypted), the HTTP response from
    // sendMessageToConversation already stores the correct plaintext. Skip the
    // fetch-and-decrypt path here to avoid overwriting it with a failed decryption.
    if (isOwn && !displayContent) {
      return;
    }
    ...
    // If the message content is empty it means the message is encrypted.
    // The WebSocket event does not carry ciphertext/iv/wrapped_key, so we
    // must re-fetch the conversation from the REST API to decrypt it properly.
    if (!displayContent) {
      if (directUserId) {
        this.fetchConversationMessages(conversationId, 'direct', Number(directUserId));
      } else {
        this.fetchConversationMessages(conversationId, 'room', conversationId);
      }
    }
